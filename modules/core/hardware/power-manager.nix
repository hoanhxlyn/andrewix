{lib, ...}: {
  core.power-manager = {
    nixos = {config, ...}: {
      ...
      # Allow wheel to flip TLP power profiles without a password. Scoped to the
      # exact three subcommands; sudo already has a working setuid wrapper here
      # (pkexec does not), so this is the reliable escalation path.
